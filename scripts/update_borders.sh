...
tmux list-panes -F "#{pane_id} #{pane_index} #{pane_current_path} #{pane_width}" | while read -r pane_id pane_index pane_current_path pane_width; do
  (
    ## Build the pane_border_format
    ...
    ## Apply
    tmux set-option -p -t "$pane_id" pane-border-format "$pane_border_format"
  ) &
done
wait